...
export default function HomePage() {
  const { tasks, notes, userId, isGuest, fetchUserData } = useData();
  ...
  useEffect(() => {
    if (tasks && notes) {
      ...
      const updatedPinnedNotes = notes.filter(note => note.is_pinned);
      const updatedPinnedTasks = tasks.filter(task => task.is_pinned);
  ...
  }, [tasks, notes]); // React to changes in tasks and notes
